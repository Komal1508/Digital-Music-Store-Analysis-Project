...
select customers.customerId , FirstName, LastName, sum(Total) as Total_spending
from customers
...
FROM customers
JOIN invoices ON customers.customerId = invoices.customerId
JOIN invoicelines ON invoices.invoiceId = invoicelines.invoiceId
WHERE TrackId IN(
...
JOIN artist ON artist.ArtistId = albums.ArtistId
